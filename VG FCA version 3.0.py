...
while i<fragment:
    ...
    C_r=(100 * random.randint(1, 9) +10 * random.randint(1, 9) + random.randint(1, 9) + random.randint(1, 9) / 10 + random.randint(1, 9) / 100) / (
                    10 ** random.randint(2, 6))
    try:
        x = [TS_r, TR_r, A_r, B_r,C_r]
        ts = x[0]
        tr = x[1]
        a = x[2]
        b = x[3]
...
"""clustring"""
if len(TSF)>1:
    selected_fragment_array = np.stack((TSF, TRF, AF, BF, CF), axis=-1)
    i = 1
    J = 0
